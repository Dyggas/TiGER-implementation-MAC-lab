...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[
        node distance=1.6cm,
        >=Stealth,
        every node/.style={draw, minimum width=7cm, align=center},
        terminal/.style={rounded corners=20pt, minimum height=1.6cm, font=\small},
        box/.style={rectangle, minimum height=1.2cm, font=\bfseries\large}
    ]

        \node[terminal] (amylase) {
            Амілаза більш ніж в 3 рази перевищує верхню \\
            межу рівня амілази в сироватці крові \\
            в даному лікувальному закладі
        };

        \node[box, fill=green!50, below=of amylase] (bl) {
            Біохімічний витік
        };

        \node[terminal, below=of bl, minimum height=2.5cm, text width=8.5cm, font=\footnotesize] (criteria) {
            - Постійний дренаж > 3-х тижнів* \\
            - Клінічно значуща зміна в лікуванні ПОПН \\
            - Черезшкірне або ендоскопічне дренування* \\
            - Ангіографічні втручання при кровотечі* \\
            - Ознаки інфекції без органної недостатності*
        };

        \node[box, fill=yellow!70, below=of criteria] (gradeB) {
            Ступінь B \\ Панкреатична нориця
        };

        \node[terminal, below=of gradeB, minimum height=1.3cm] (severe) {
            Реоперація* \\
            Органна недостатність* \\
            Смерть*
        };

        \node[box, fill=red!50, below=of severe] (gradeC) {
            Ступінь C \\ Панкреатична нориця
        };

        % Arrows
        \draw[->, very thick] (amylase) -- (bl);
        \draw[->, very thick] (bl) -- (criteria);
        \draw[->, very thick] (criteria) -- (gradeB);
        \draw[->, very thick] (gradeB) -- (severe);
        \draw[->, very thick] (severe) -- (gradeC);

        % Footer
        \node[draw=none, below=0.5cm of gradeC, font=\small\itshape] {*Лікування/подія, пов'язана з ПОПН};

    \end{tikzpicture}
    \caption{Classification of Pancreatic Fistula}
\end{figure}
% TODO: Заповнити розділ
