...
\end{equation*}
де $h(x) = (f \cdot g)(x) = \sum\limits_{i=0}^{n-1} h_i x^i$.
...
\begin{itemize}
    \item \textbf{Failure boosting attacks~\cite{danvers2019failure}:} Супротивник може ітеративно створювати 
        шифротексти, що мають високу ймовірність помилки, щоб витягувати поступово інформацію про секретний ключ;
    \item \textbf{Multi-target attacks~\cite{graham2003}:} При наяності багатьох публічних ключів або сесій,
...
\begin{definition}[Quantum Random Oracle Model~\cite{hovelmanns2020}]
    \label{def:3-5-6}
...
Є три основні підвиди атак на бічні канали (детальніша класифікація у розділі~\ref{sec:7-7}):
...
Alexander May розробив спеціалізований варіант MitM для задачі LWE~\cite{meetlwe2021}, відомий як Meet-LWE атака~\cite{may2021}.
...
В 2024 році von Berg~\cite{vonberg2023} та його колеги опублікували роботу, що аналізує CCA атаку на PKE схеми на основі решіток, включаючи 
...
Припустимо, що атакуючий може спостерігати не лише фінальне декодоване повідомлення $\mu'$ (після застосування кодів 
корекції), але й \textit{проміжний результат} $\mathbf{v}_{\text{raw}}$ \textit{до} застосування кодів корекції помилок:
\begin{equation*}
...
коректної його реалізації. Атака von Berg (повна назва: CCA attack on PKE with access to intermediate decryption) 
на базову PKE не застосовна до повної схеми KEM. Основні ризики пов'язані з side-channel атаками, які потребують 
обережної імплементації з константним часом та маскуванням. Незважаючи на застосування модифікованого FO перетворення, 
деякі практичні атаки все ще можливі:
...
\section{Атака з доступом до проміжного виводу декодування}
\label{sec:7-5}

Ще доцільно поговорити про специфічний клас атак, коли противник може спостерігати або маніпулювати проміжними 
значеннями під час процесу дешифрування. У розділі~\ref{sec:7-4} ми вже поверхнево зачіпали і наводили приклад 
такої атаки (von Berg). Також приділимо увагу унікальній структурі TiGER, що використовує коди корекції помилок 
XEf та D2.

\subsection*{Етапи дешифрування в TiGER}

Визначимо спершу позначення:
\begin{itemize}
    \item $\mu$ -- оригінальне повідомлення (зашифроване відправником)
    \item $\tilde{\mu}$ -- необроблені біти після екстракції (можуть містити помилки)
    \item $\hat{\mu}$ -- після XEf корекції
    \item $\mu'$ -- після D2 корекції (фінальний результат)
\end{itemize}

\noindent Процес декапсуляції TiGER складається з кількох етапів та відповідних проміжних значень:

\begin{enumerate}[label = \arabic*)]
    \item Декомпресія шифротексту:
        \begin{equation*}
    ...
    \item Обчислення проміжного значення $\mathbf{v}$:
        \begin{equation*}
            \mathbf{v}_{\text{raw}} = \mathbf{c}'_2 - \mathbf{c}'_1 \cdot \mathbf{s}
        \end{equation*}
        Це значення містить \textit{закодоване оригінальне повідомлення} $\mu$ плюс накопичені помилки з минулих 
        від етапів (RLWE, RLWR та компресії).
    \item Екстракція помилкових бітів:
        \begin{equation*}
            \tilde{\mu}_i = \text{ExtractBit}(\mathbf{v}_{\text{raw}}, i), \quad i = 0, \ldots, 255
        \end{equation*}
        (!) Кожен біт $\tilde{\mu}_i$ може бути помилковим через надмірні попередні помилки. Це так звані необроблені 
        біти.
    \item Корекція помилок за допомогою XEf:
        \begin{equation*}
    ...
    \item Корекція помилок за допомогою D2:
        \begin{equation*}
            \mu' = \text{D2.Decode}(\hat{\mu}), \quad \text{(мажоритарне голосування для кожного біту)}
        \end{equation*}
        D2 виправляє помилки, використовуючи дуплікат інформації. Результатом є $\mu'$ -- декодоване повідомлення 
        (в ідеалі має дорівнювати оригінальному $\mu$).
    \item Повторне шифрування (FO check):
        \begin{equation*}
            ct' = \text{PKE.Encrypt}(pk, \mu'; H(\mu'))
        \end{equation*}
        Також робиться перевірка: $ct' \stackrel{?}{=} ct$. Якщо рівність правильна, то повертається $K = H(\mu')$, 
        інакше -- $\bar{K} = H(z, ct)$ (застосовується implicit rejection).
\end{enumerate}

\subsection*{Вразливість через витік проміжних значень}

Припустимо, що атакуючий може спостерігати $\tilde{\mu}$ (біти після екстракції, але \textit{до} застосування кодів 
корекції) через side-channel (power analysis, EM, timing) -- все як в атаці von Berg. Постає логічне питання: Що 
дає атакуючому $\tilde{\mu}$? З рівняння, яке записували для $\mathbf{v}_{\text{raw}}$ маємо:
\begin{equation*}
    \mathbf{v}_{\text{raw}} = \mathbf{c}'_2 - \mathbf{c}'_1 \cdot \mathbf{s} = \underbrace{\mathbf{b} \cdot 
    \mathbf{e}_1 + \mathbf{e}_3}_{\substack{\text{відомі або} \\ \text{контрольовані}}} + 
    \underbrace{\text{Encode}(\mu)}_{\text{(не)відомий}} - \mathbf{c}'_1 \cdot \mathbf{s}
\end{equation*}

\noindent Перегрупуємо:
\begin{equation*}
...
\begin{itemize}
    \item  \textit{Сценарій 1 ("пасивний"):} Атакуючий перехоплює чужий шифротекст, де $\mu$ невідомий. Його мета -- 
        витягнути $\mu$ з спостережень $\mathbf{v}_{\text{raw}}$.
    \item \textit{Сценарій 2 ("активний"):} Атакуючий сам створює шифротексти з відомими йому $\mu^{(i)}$ і 
        спостерігає відповідні $\mathbf{v}_{\text{raw}}^{(i)}$. Його мета вже змінюється, він хоче витягнути 
        $\mathbf{s}$ розв'язавши систему лінійних рівнянь.
\end{itemize}
Розглянемо другий випадок:
\begin{enumerate}
    \item Атакуючий створює $m$ шифротекстів, самостійно обираючи помилки $\mathbf{e}_1^{(i)}, \mathbf{e}_3^{(i)}$ 
        та повідомлення $\mu^{(i)}$;
    \item Для кожного шифротексту отримує $\tilde{\mu}^{(i)}$, з яких може обчислити вже $\mathbf{v}_{\text{raw}}^{(i)}$;
    \item Оскільки атакуючий знає всі $\mu^{(i)}$, то він може обрахувати і кожен $\text{Encode}(\mu^{(i)})$. 
        Отримується система рівнянь:
        \begin{equation*}
            \mathbf{c}'^{(i)}_1 \cdot \mathbf{s} = \underbrace{(\mathbf{b} \cdot \mathbf{e}_1^{(i)} + \mathbf{e}_3^{(i)} + \text{Encode}(\mu^{(i)}) - \mathbf{v}_{\text{raw}}^{(i)})}_{\text{відомі, оскільки атакуючий сам обрав }\mu^{(i)}} + \mathbf{e}_{\text{noise}}^{(i)}
        \end{equation*}
        де $\mathbf{e}_{\text{noise}}^{(i)}$ -- залишкова помилка від RLWR та округлень;
    \item Маючи $m \geq n$ рівнянь виду $\mathbf{C}_1 \cdot \mathbf{s} = \mathbf{y} + \mathbf{E}_{\text{noise}}$, 
        розв'язує їх будь-яким зручним LWE-solving методом, знаходячи $\mathbf{s}$.
\end{enumerate}

\noindent Необхідна кількість запитів до оракула для повного витягування $\mathbf{s}$ з $n = 512$ коефіцієнтами 
складає приблизно:
\begin{equation*}
    m \approx n + O(\sqrt{n}) \approx 512 + 50 = 562
\end{equation*}

\noindent Після збору $m$ зразків, розв'язування системи рівнянь матиме складність:
\begin{equation*}
    \text{Complexity} \approx O(m^3) \approx O(512^3) \approx 2^{27} \text{ операцій}
\end{equation*}
Це \textit{практично здійсненна} атака, якщо є витік $\tilde{\mu}$.

\noindent В першому ж випадку $\mu$ є невідомим і атака була б набагато складнішою.

TiGER має закладений захист: коди корекції XEf і D2 приховують зв'язок між $\tilde{\mu}$ та фінальним $\mu$. Проблема 
для атакуючого полягає в тому, що щоб використати спостережені $\tilde{\mu}_{\text{received}}$ для витягування $\mathbf{s}$, 
потрібно знати \textit{оригінальне} $\mu$. Але зв'язок між $\mu$ та $\tilde{\mu}_{\text{encoded}}$ є нетривіальним 
через те, що помилки змінюють кожен біт непередбачувано, а коди XEf та D2 не обов'язково виправлять усі існуючі 
помилки (про це у наступному розділі~\ref{sec:7-6}). Тому без знання точного $\mu$, атакуючий не може побудувати 
систему рівнянь.

Але що буде, якщо атакуючий дізнався частину $\mu$? Чи допоможе це йому? Припустимо, що через якусь іншу вразливість 
зловмисник дізнався $k$ біт з $\mu$. Чи полегшить це атаку? Частково так, але залишається $256 - k$ невідомих біт, 
а коди корекції розподіляють інформацію, тому навіть знаючи $k$ біт $\mu$, важко точно передбачити всі біти $\tilde{\mu}$. 
Нехай $k = 128$ біт, $\mu = 256$, тоді залишкова ентропія складає $2^{128}$. Це все ще занадто велике для перебору 
і алгоритм залишається захищеним.

\begin{table}[H]
    \centering
    \begin{tblr}{
        colspec = {Q[l, 3.7cm]Q[l, 12cm]},
        vlines,
        hlines,
        row{1} = {c},
        }
        \textbf{Міра захисту}     & \textbf{Ефект}                                                  \\
        \hline
    ...
    \end{tblr}
    \caption{Захист TiGER від атак на проміжні значення}
    \label{table:7-5-1}
\end{table}

Атака через витік проміжних значень декодування є теоретично можливою, але TiGER має багаторівневий захист. Коректна 
реалізація (з усіма мірами захисту) алгоритму майже унеможливлює цю атаку.

\section{Атака з урахуванням XEf корекції помилок}
\label{sec:7-6}

Використання коду корекції помилок XEf є особливістю TiGER, що дозволяє здійснювати агресивнішу компресію шифротексту. 
Тут, в цій секції, розглянемо чи можна якось експлуатувати структуру XEf для атак на TiGER.

\noindent Параметри XEf, які застосовуються для TiGER:
\begin{itemize}
    \item Розмір блоку: $d = 8$ біт
    \item Кількість блоків: $256 / 8 = 32$ блоки
    \item Можливість корекції: $t = 1$ помилка на блок
\end{itemize}

\subsection*{Алгоритм застосування XEf}

Спершу відбувається кодування:

Для оригінального блоку даних $\mathbf{m} = (m_0, m_1, \ldots, m_7) \in \{0,1\}^8$ обчислюється так званий 
синдром $s$, шляхом застосування операції XOR:
\begin{equation*}
    s = m_0 \oplus m_1 \oplus \ldots \oplus m_7 = \bigoplus\limits_{i=0}^{7} m_i
\end{equation*}

\noindent Декодування проходить наступним чином:

Маємо отриманий блок (received block) $\mathbf{y} = (y_0, \ldots, y_7) = \mathbf{m} \oplus \mathbf{e}$ може містити 
помилки $\mathbf{e}$. Обчислюємо значення:
\begin{equation*}
    s' = \bigoplus\limits_{i=0}^{7} y_i
\end{equation*}

\noindent Якщо $s' \neq s$, то присутня помилка. XEf визначає позицію помилки та виправляє її.

\subsection*{Теоретична вразливість XEf}

\textbf{Проблема 1: Обмежена здатність корекції}

XEf виправляє лише $t = 1$ помилку на блок. Якщо в блоці $\geq 2$ помилки, то:
\begin{equation*}
    \mathbb{P}[\text{блок декодується неправильно}] \approx C_{8}^{2} \cdot p^2 = 28p^2
\end{equation*}
де $p$ -- ймовірність помилки одного біту.

\noindent \textbf{Проблема 2: Детермінована структура}

Оскільки синдром є лінійною функцією від бітів блоку $f(\mathbf{m})$, то знаючи деякі біти, можна обчислити синдром 
$s$ та використати цю інформацію для звуження простору можливих $\mathbf{m}$.

\subsection*{Атака через маніпуляцію синдромами}

Нехай зловмисник вміє перехоплювати шифротекст $ct$, модифіковувати його для індукування помилок, а також спостерігати, 
чи була декапсуляція успішною.

Ідея атаки полягає в наступному:

\begin{enumerate}
    \item Зловмисник flip-ує (модифікує) один біт у блоці шифротексту:
        \begin{equation*}
            ct'_i = ct_i \oplus (1 \ll j), \quad j \in \{0, \ldots, 7\}
        \end{equation*}
    \item Це призводить до зміни одного біту в певному блоці після дешифрування;
    \item XEf код спробує виправити цю помилку: якщо в блоці більше немає помилок, XEf успішно виправить і декапсуляція 
        буде успішною;
    \item Якщо в блоці вже була 1 помилка (наприклад від компресії/RLWE), то тепер їх 2. XEf виправляє одну, а інші 
        лишаються і декапсуляція є неуспішною;
    \item Спостерігаючи success/failure для різних flip-ів, атакуючий може дізнатися про розподіл помилок у блоках.
        Це дає приблизно $\log_2 3 \approx 1.58$ біт інформації на кожен блок (три стани: 0 помилок, 1 помилка, 
        $\geq 2$ помилок).
\end{enumerate}

\noindent Для 256 бітного повідомлення (займає 32 блоки), маємо:
\begin{equation*}
    \text{Total leaked info} \approx 32 \cdot 1.58 \approx 50 \text{ біт}
\end{equation*}
Цього для компрометації і витягування 785-бітного секретного ключа або 256-бітного повідомлення недостаньо, проте 
це зменшує простір пошуку з $2^{256}$ до $2^{206}$.

Оскільки TiGER використовує FO$^{\not\bot}_m$ з implicit rejection, що унеможливлює спостереження success/failure, 
це зводить цю атаку нанівець.

\subsection*{Атака спираючись на алгебраїчні властивості XEf}
...
\begin{enumerate}
    ...
    \item Без знання помилок, система лінійних рівнянь на синдроми має занадто багато невідомих:
        \begin{itemize}
...
\noindent Атаки, що спрямовані на XEf код корекції помилок є небезпечними, але для TiGER практично нездійсненними 
...
енергії, електромагнітне випромінювання замість безпосередньої атаки на математичну структуру. Для TiGER, як і для 
всіх постквантових алгоритмів, side-channel атаки становлять серйозну практичну загрозу~\cite{ravi2020}.

\subsection*{Класифікація side-channel атак}
У пункті 5 розділу~\ref{sec:7-1} ми вже частково згадували за них. Зараз більш детально розглянемо кожну з них:
...
\subsection*{Timing атаки на TiGER}

Перше, що є важливим джерелом variable timing -- це множення многочленів на розріджені секрети. TiGER 
використовує секрет $\mathbf{s}$ з вагою Геммінга $h_s = 274$. Схематично в реалізації це виглядає так:
\begin{verbatim}
    for i in 0..n-1:
        if s[i] != 0:
            result += a[i] * s[i]  // Executed h_{s} times
\end{verbatim}
Час виконання: $T \approx h_s \cdot T_{\text{mul}}$ варіюється \textit{залежно від позицій} ненульових коефіцієнтів.
...
\begin{verbatim}
    if syndrome_matches:
        ...
        keep_as_is()
\end{verbatim}
Час залежитиме від кількості помилок у блоках.

Як використати цю інформацію? Зловмисник виміряв час декапсуляції для різних шифротекстів та побудував статистичну 
модель:
\begin{equation*}
    T_{\text{decaps}}(ct) \approx T_{\text{base}} + \sum\limits_{i} w_i \cdot s_i + \text{noise}, \quad \text{де}
\end{equation*}

\begin{itemize}
    \item $T_{\text{base}}$ -- базовий час виконання операцій, які не залежать від секретного ключа;
    \item $s_i$ -- $i$-тий коефіцієнт секретного ключа (attack target values);
    \item $w_i$ -- ваги, що корелюють з операціями на $s_i$, іншими словами це скільки додаткового часу додає 
        коефіцієнт $s_{i}$ до загального часу виконання;
    \item $\text{noise}$ -- шум вимірювань.
\end{itemize}

Маючи достатню кількість вимірювань ($\approx 10^4$-$10^6$), можна відновити $\mathbf{s}$ методами machine learning 
або кореляційного аналізу.

Захищатися від цього можна маючи константний час виконання (додавання dummy operations/masking) або (а краще і) 
випадковізація (shuffle) порядку обчислень (при тому ж множенні наприклад)

\subsection*{Power analysis (DPA/CPA)}

\begin{enumerate}
    \item Differential power analysis (DPA): Зловмисник бачить на екрані осцилографа трейси споживання енергії під 
        час множення $\mathbf{a} \cdot \mathbf{s}$:
        \begin{enumerate}[label = \arabic*)]
            \item Він збирає $N$ трейсів споживання енергії для різних $\mathbf{a}^{(i)}$;
            \item Будує по три гіпотези для кожного біту $s_j \in \{-1, 0, 1\}$;
            \item Обчислює кореляцію між гіпотетичним споживанням та виміряним:
                \begin{equation*}
                    \rho_{j, \text{hyp}} = \text{corr}(\mathbb{P}_{\text{measured}}, \mathbb{P}_{\text{model}}(s_j = \text{hyp}))
                \end{equation*}
            \item Вибирає гіпотезу з найвищою кореляцією як правильну.
        \end{enumerate}
    \item Correlation Power Analysis (CPA): Використовуючи Hamming weight model:
        \begin{equation*}
            P_{\text{model}}(x) \approx \alpha + \beta \cdot HW(x), \quad \text{де}
        \end{equation*}
        \begin{enumerate}[label = \arabic*)]
            \item $P_{\text{model}}(x)$ -- очікуване споживання енергії пристрою під час обробки значення $x$, 
                вимірюється в одиниці потужності (Вт) або напругою на осцилографі
            \item $\alpha$ -- базове споживання енергії пристроєм
            \item $\beta$ -- коефіцієнт, що показує залежність від даних (скільки додаткової енергії споживає один 
                біт, встановлений в 1)
            \item $HW(x)$ -- кількість одиниць у двійковому представленні $x$.
        \end{enumerate}
\end{enumerate}
Яка ефективність всього цього проти TiGER? Оскільки $\mathbf{s}$ розріджений, атакуючий може зосередитися на 
виявленні позицій ненульових коефіцієнтів. Оскільки, грубо кажучи:
\begin{itemize}
    \item Якщо $s_i = 0$: операція $a_i \cdot s_i$ майже не споживає енергії;
    \item Якщо $s_i = \pm 1$: помітне споживання.
\end{itemize}
За допомогою ідентифікації позиції ненульових $s_i$ можна трохи зменшити простір пошуку справжнього ключа. А 
захиститися від цього можна шляхом Masking 1/2-го порядку: розділивши $\mathbf{s} = \mathbf{s}_0 \oplus \mathbf{s}_1$ 
на дві/більше випадкових частинок.

\subsection*{Cache timing атаки}

Механізм атаки закручений на архітектурі сучасних процесорів. Вони мають ієрархічну пам'ять (L1/L2/L3 кеш, RAM). 
Доступ до кешу відбувається набагато швидше за RAM, тому зловмисник може:
\begin{enumerate}
    \item Заповнити кеш своїми даними (FLUSH);
    \item Викликати функцію TiGER, що працює з секретом;
    \item Виміряти, які кеш-лінії були витіснені (зробити RELOAD);
    \item З паттернів доступу дізнатися інформацію про секрет.
\end{enumerate}

\noindent Вразливість TiGER заключається в тому, що якщо індекси ненульових коефіцієнтів $\mathbf{s}$ використовуються 
для доступу до таблиць (наприклад, multipication lookup tables), то паттерн доступу спричиняє витік інформації:
\begin{verbatim}
    for i in sparse_indices(s):  // Indices of non-zero s_i
        result += lookup_table[a[i]]  // Access at index i
\end{verbatim}

Як результат атакуючий бачить, які $i$ були використані і дізнається позиції ненульових коефіцієнтів.
Захиститися від цього дуже просто:
\begin{itemize}
    \item Завжди обробляти всі індекси, використовуючи conditional move:
        \begin{verbatim}
            for i in 0..n-1:
                mask = (s[i] != 0)
                result += mask * lookup_table[a[i]]
        \end{verbatim}
    \item Уникати звернтання по індексу, натомість використовувати лише арифметичні операції;
    \item Випадковізувати порядок доступу до пам'яті (Memory access randomization).
\end{itemize}

\subsection*{Fault injection атаки}

Fault injection attacks поділяються ще на чотири підкатегорії:
\begin{enumerate}
    \item \textbf{Voltage glitching:} Короткочасне зниження напруги $\to$ пропуск операції;
    \item \textbf{Clock glitching:} Маніпуляція тактовою частотою CPU $\to$ неправильні обчислення;
    \item \textbf{Laser injection:} Цілеспрямоване випромінювання $\to$ bit flips у регістрах/пам'яті;
    \item \textbf{EM injection:} Електромагнітні імпульси $\to$ індукування сторонніх помилок.
\end{enumerate}

\noindent Мета цієї атаки на TiGER полягає в наступному -- обійти re-encryption check у FO перетворенні, щоб змусити 
алгоритм повернути детерміністичний ключ $K = H(m, ct)$ замість псевдовипадкового $\bar{K} = H(z, ct)$ для 
модифікованого шифротексту $ct'$.

\begin{enumerate}
    \item Зловмисник модифікує шифротекст $ct$ (наприклад, змінює один біт) і має $ct'$;
    \item Індукує fault під час виконання re-encryption check:
        \begin{verbatim}
            m' = Dec(sk, ct)
        ...
        \end{verbatim}
        Якщо fault успішний (перевірка не виявила підхову), то алгоритм повернув K = H(m', ct) -- детермінований 
        ключ, що залежить від декодованого $m'$. Для того самого $ct'$, ключ $K$ буде однаковим при повторних запитах.
    \item Зловмисник може експлуатувати це так:
        \begin{itemize}
            \item Подає $ct'$ з різними fault injection багато разів;
            \item Якщо отримує однакові відповіді для $ct'$, то fault успішний, отримано детермінований $K$;
            \item Якщо отримує різні відповіді, то fault не спрацював, отримував $\bar{K} = H(z, ct')$ з 
                випадковим $z$;
            \item Маючи детермінований $K$ для модифікованого $ct'$, він може отримати якусь інформацію про 
                $m'$ через side-channel аналіз або статистичні атаки.
        \end{itemize}
\end{enumerate}

\noindent Захист у TiGER є наступним:

\begin{enumerate}
    \item Використання implicit rejection ускладнює fault атаку, але \textit{не є} повноцінним захистом(!)
    \item Використання кодів корекції (XEf + D2): Можуть виправити деякі індуковані помилки у даних $ct$ до 
        початку декапсуляції;
\end{enumerate}

\noindent З того, що бажано б додати для додаткового захисту, це:
\begin{enumerate}
    \item Виконувати re-encryption check двічі незалежними способами:
        \begin{verbatim}
            ct'' = Enc(pk, m'; G(m', H(pk)))
            check1 = (ct'' == ct)
    ...
    \item Проводити моніторинг скачків напруги, тактової частоти для виявлення fault injection спроб;
\end{enumerate}

Підсумовуючи, можна сказати що: side-channel атаки є найсерйознішою практичною загрозою для TiGER. Математична 
конструкція алгоритму стійка, але паршива реалізація може призводити до витоків секретів через фізичні канали. 
Для безпечного використання TiGER критично важливі:
\begin{enumerate}
    \item Константний час усіх операцій;
    \item Masking секретних даних (принаймні 1-го порядку);
    \item Уникнення secret-dependent memory access (див. cache timing attacks);
    \item Захист від fault injection через \textit{подвійні перевірки};
    \item Апаратні контрзаходи (HSM, secure enclaves).
\end{enumerate}

\section{Перенесення атак з RLWE/RLWR схем на TiGER}
\label{sec:7-8}

TiGER є частиною великої родини криптосистем на основі RLWE/RLWR. Багато атак, розроблених для інших схем 
(Kyber, Saber, NTRU, NewHope), можуть потенційно бути адаптовані до TiGER.

\subsubsection*{1. Решіткові атаки (Primal/Dual Lattice Attack)}

\textbf{Статус:} \textcolor{orange}{Застосовна, але неефективна}

Всі схеми на основі RLWE/RLWR вразливі до цих решіткових атак, але оскільки ця атака універсальна вона не є 
специфічною. Та й TiGER використовує консервативно обрані параметри з урахуванням найновіших (на 2022 рік) оцінок 
складності BKZ (Core-SVP $\beta = 483$).

\subsubsection*{2. Meet-LWE атака}

\textbf{Статус:} \textcolor{orange}{Частково застосовна}

Розроблена для LWE з розрідженими секретами, прямо застосовна до TiGER. але Квантова складність 
$\approx 2^{109.5}$ хоч і нижче цільового рівня 128 біт, проте, потребує нереалістичних ресурсів (пам'ять) для 
самої атаки.

\subsubsection*{3. Failure boosting атака}

\textbf{Статус} \textcolor{Green4}{не застосовна}

Оригінальна атака~\cite{danvers2019failure} застосовувалася до Kyber, Saber та LAC схем без implicit rejection. 
Оскільки TiGER використовує FO$^{\not\bot}_m$ з implicit rejection, то немає observable $\bot$ і атакуючий не знає, 
чи відбувалися failure.

\subsubsection*{4. CCA атаки на базову PKE}

\textbf{Статус:} \textcolor{Green4}{не застосовна}

Атаки по типу von Berg базуються на відсутності верифікації цілісності $ct$ у базових PKE схемах. А TiGER 
використовує повне FO перетворення з re-encryption check, тому будь-яка модифікація $ct$ виявляється та атака 
блокується.

\subsubsection*{5. Signal leakage атаки}

\textbf{Статус:} \textcolor{orange}{Теоретично застосовна (якщо реалізація хромає), але практично складна}

Оригінальна атака~\cite{guo2019} застосовувалась до схем Round5, LAC, де проміжні "сигнали"{} могли витікати 
через side-channels і за великої кількості спостережень можна витягнути $\mathbf{s}$.

Якщо $\mathbf{v}_{\text{raw}}$ витікає через power/EM side-channel, атака можлива (див. секцію 7.5). Проте 
належна реалізація з masking мітигує цю атаку.

\subsubsection*{6. NTRU-specific атаки (не RLWR/RLWE схема, але ладно)}

\textbf{Статус:} \textcolor{Green4}{не застосовна}

\noindent NTRU використовує структуру кільця $\mathbb{Z}[x]/(x^n - 1)$ та інші методи множення (FFT/NTT), в той час як 
TiGER використовує циклотомічне кільце $\mathbb{Z}_q[x]/(x^n + 1)$.

\subsubsection*{Lesson learned from predecessors:}

\begin{enumerate}
    \item \textbf{Від Kyber:} Консервативний вибір параметрів безпеки, аналіз DFP;
    \item \textbf{Від Saber:} LWR підхід для компактності, уникнення NTT;
    \item \textbf{Від LAC/Round5:} Коди корекції помилок (XEf);
    \item \textbf{Від досліджень failure boosting:} Implicit rejection;
    \item \textbf{Від NewHope:} Importance of reconciliation / error correction for reliability.
\end{enumerate}

\noindent Основні ризики залишаються в площині side-channel атак, які потребують лишень обережної реалізації.

\chapter{\large{Практичне застосування та висновки}}
\label{chapter:8}

\section{Можливі сценарії використання TiGER}
\label{sec:8-1}

TiGER, як постквантовий KEM алгоритм, може інтегруватися у широкий спектр криптографічних протоколів та застосувань: 
\begin{itemize}
    \item TLS/HTTPS: Використання TiGER для встановлення спільного ключа в TLS 1.3 може потенційно захистити 
        веб-комунікації від квантових атак.
    \item VPN та IPsec: Інтеграція TiGER в IKEv2/IPsec протоколи забезпечуватиме квантовостійке шифрування 
        для VPN-з'єднань.
\end{itemize}

\section{Підсумки дослідження}
\label{sec:8-2}

Ми комплексно дослідили (хочеться вмерти після цього) постквантовий криптографічний алгоритм TiGER, що базується 
на проблемах RLWE та RLWR, його теоретичне підґрунтя та параметри безпеки.

\noindent \textbf{Основні результати:}
\begin{itemize}
    \item Побачили, що TiGER забезпечує всі рівні безпеки NIST;
    \item Подвійна система корекції помилок (XEf + D2) та implicit rejection забезпечують стійкість до більшості 
        відомих атак;
    \item Компактні розміри ключів та шифротекстів роблять TiGER привабливим для практичних застосувань;
    \item Основні ризики пов'язані з side-channel атаками, вимагають обережної реалізації системи.
\end{itemize}

TiGER демонструє збалансоване поєднання безпеки, ефективності та практичності, що підтверджує його потенціал як 
постквантової криптографічної системи. Злиття з SMAUG у алгоритм SMAUG-T та його визнання фіналістом KpqC 
підкреслює перспективність підходу.

Найближчі роки будуть вирішальними для формування постквантового криптографічного ландшафту, тому своєчасна 
міграція критичної інфраструктури є необхідною для захисту від майбутніх квантових загроз.
